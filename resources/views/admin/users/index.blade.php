...
@section('content')
    ...
    <div style="background:linear-gradient(45deg, #f33057, rgb( 56, 88, 249 ) );"
        class="d-sm-flex d-block align-items-center justify-content-between page-header-breadcrumb mb-2">
        <div>
            <h4 class="fw-medium mb-2">User</h4>
            <div class="ms-sm-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="javascript:void(0);">User</a></li>
                        <li class="breadcrumb-item active fw-normal" aria-current="page">User List</li>
                    </ol>
                </nav>
            </div>
        </div>
                ...
                <div class="row align-items-center">
                    <div class="col">
                        <div class="breadcrumb mt-3 border-bottom pb-2">
                            <a href="{{ url('') }}"><i class="fa fa-home" aria-hidden="true"></i> Home</a>/Users List
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <div class="card-body booking_card">
                                    ...
                                    <div class="table-responsive">
                                        <table id="hotel_table"
                                            class="table table-stripped table-hover table-center mb-0">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Name</th>
                                            ...
                                            <tbody>
                                                @foreach ($users as $key => $value)
                                                    <tr>
                                                        ...
                                                        <td>{{ $value->method }}</td>
                                                        <td>{{ $value->payment_vpa }}</td>
                                                        {{-- <td>{{ $value->user_type_name }}</td> --}}
                                                        {{-- <td>
                                                            <select class="change_status" name="status"
                                                                data-user_id="{{ $value->id }}">
                                                                <option value="1" class="text-success"
                                                                    @if ($value->status == 1) selected @endif>Active
                                                                </option>
                                                        ...
                                                        <td>{{ convert_datetime_to_date_format($value->created_at, 'd M Y') }}
                                                        </td>
                                                        {{-- <td class="text-right">
